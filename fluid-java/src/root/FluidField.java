...
public class FluidField {
	...
	public void project(double dt){
		...
		for(int y = 0; y <= _size/2; y++){
			for(int x = 0; x < _size/2; x++){
				...
				// Project Coefficient onto Wavenumber Vector
	...
	public void impulse(double fx, double fy, int x, int y, boolean drawDensity){
		int windowSize = 20;
	...
	public void renderDensity(BufferedImage bi){
		for(int x=0; x < _size; x++){
			for(int y=0; y< _size; y++){
				...
				int red = (int) ((1-density) * 255);
				int blue = (int) ((1-density) * 255);
				int green = (int) ((1-density) * 255);
				int rgb = (red<<16) | (green<<8) | blue;
	...
	public void renderFlow(BufferedImage bi, double scaleFactor){
		...
		g.setColor(Color.BLACK);  //g.setColor(new Color(1f, 1f, 1f, 0.5f));
		for(int x=0; x < _size; x += 10){
			for(int y=0; y < _size; y += 10){
				...
				double ux = velX/mag;
				double uy = velY/mag;
				velX = Math.sqrt(Math.abs(velX)) / 1.5;
				velY = Math.sqrt(Math.abs(velY)) / 1.5;
